...
function fixture(options = {}) {
  ...
  const client = {
    async query(text, parameters) {
      ...
      if (options.fail?.(text)) throw new Error("private identity database detail");
  ...
  return { calls, database: new PostgresIdentityDatabase({ connect: async () => client }) };
}

test("identity database fixes the dedicated NOBYPASSRLS role and clears app scope before reads", async () => {
  const { database, calls } = fixture();
  await database.transaction((tx) => tx.query("SELECT 1"));
  assert.ok(calls.some((call) => call.text === "SET LOCAL ROLE sbg_identity_service_rw"));
  assert.ok(calls.some((call) => String(call.text).includes("set_config('app.tenant_id'")));
  assert.deepEqual(calls.at(-1), { release: false });
});

test("identity database refuses unsafe login/runtime role evidence", async () => {
  const { database } = fixture({ safe: false });
  await assert.rejects(database.transaction(async () => {}),
    (error) => error.code === "DATABASE_ROLE_UNSAFE");
});

test("identity database destroys a connection when cleanup fails", async () => {
  const { database, calls } = fixture({ fail: (text) => String(text).startsWith("RESET app.") });
  assert.equal(await database.transaction(async () => "ok"), "ok");
  assert.deepEqual(calls.at(-1), { release: true });
});

test("identity transaction handles expire after callback completion", async () => {
  const { database } = fixture();
  let leaked;
  await database.transaction(async (tx) => { leaked = tx; });
  await assert.rejects(leaked.query("SELECT 1"),
