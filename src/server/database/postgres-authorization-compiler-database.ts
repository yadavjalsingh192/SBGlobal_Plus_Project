...
/** Dedicated Authorization-compiler SQL boundary. The fixed role can publish
 * compiled subjects/snapshots only; source Authorization truth remains outside it. */
export class PostgresAuthorizationCompilerDatabase implements SqlDatabase {
  ...
  async transaction<T>(work: (transaction: SqlTransaction) => Promise<T>): Promise<T> {
    ...
    try {
      ...
      await client.query("SET LOCAL ROLE sbg_authorization_compiler_rw");
